...
Чтобы добавить подсказки команд для бота, используйте команду `/setcommands`. Нужно написать боту список команд с описанием. Например
```text
new - Создать новую задачу
cancel - Отменить текущую команду
connect - Соеденить аккаунт с Redmine
```
